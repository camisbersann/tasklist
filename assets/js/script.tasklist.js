...
class TaskList{
    ...
    markTask(id){
        ...
        document.getElementById(id + "-button").classList.toggle("doneTask");
    }
}

const taskPost = new TaskList()
const lists = [];
let flag = -1

function list(){
    const text = document.getElementById("newtask").value;
    const task = new Task(text, generatorId(), false );

    taskPost.addTask(task);
